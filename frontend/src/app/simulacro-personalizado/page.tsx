...
export default function SimulacroPersonalizadoPage() {
  ...
  const tiemposRespuestaRef = useRef<Record<string, number>>({});
    ...
    fetch(`${API_URL}/simulacros/generar-personalizado?${params.toString()}`)
      ...
      .then((data) => {
        if (data.preguntas && data.preguntas.length > 0) {
          ...
          setError(
            data.message ??
              "No hay preguntas disponibles para lo que seleccionaste todavía.",
          );
        }
      })
  ...
  const seleccionarRespuesta = (
    ...
    if (tiemposRespuestaRef.current[preguntaId] === undefined) {
      tiemposRespuestaRef.current[preguntaId] = Math.min(
        7200,
        Math.max(
          0,
    ...
    try {
      const respuestasArray = Object.entries(respuestas).map(
        ([preguntaId, respuestaId]) => ({
          preguntaId,
          respuestaId,
          tiempoRespuestaSegundos: tiemposRespuestaRef.current[preguntaId],
        }),
      );
      ...
      if (!res.ok) {
        ...
        setError(
          data.mensaje || data.message || "No se pudo enviar el simulacro.",
        );
        setEnviando(false);
        return;
      }
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...data,
          tipo: "personalizado",
          areasSeleccionadas: areasParam.split(","),
        }),
      );
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px" }}>
        ...
        >
          ...
          {preguntaActual?.subtema?.tema?.area && (
            ...
            >
              {AREA_NOMBRES[preguntaActual.subtema.tema.area] ??
                preguntaActual.subtema.tema.area}
        ...
        >
          <ContextoCasoPregunta
            caso={preguntaActual?.caso ?? null}
            ordenEnCaso={preguntaActual?.ordenEnCaso}
          />
          <p
            style={{
              fontSize: 18,
              ...
              color: "#1a2a3a",
              lineHeight: 1.6,
              marginBottom: preguntaActual?.imagenUrl ? 20 : 32,
            }}
          >
          ...
          {esUltima ? (
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                cursor:
                  enviando || respondidas === 0 ? "not-allowed" : "pointer",
              }}
            >
